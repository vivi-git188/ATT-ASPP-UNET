...
class FetalACDataset(Dataset):
    ...
    def _build_transform(self):
        # 注意：ToTensorV2 必须最后；ToFloat 在它之前
        tf = [
            SafeCLAHE(1.0, (8, 8)),
            SafeMedianBlur(3),
        ...
        if self.train:
            # 几何 + 翻转
            tf.insert(0, ShiftScaleRotate(shift_limit=0.05, scale_limit=0.05, rotate_limit=10, p=0.3))
            tf.insert(1, HorizontalFlip(p=0.5))
            # 亮度/对比度/伽马
            tf.insert(2, RandomGamma((80, 120), p=0.5))
            tf.insert(3, RandomBrightnessContrast(p=0.3))
            # 噪声
            tf.insert(4, GaussNoise(var_limit=(5.0, 20.0), p=0.2))
        return Compose(tf)
...
def binary_dice_score(logits, targets, thresh=0.5, smooth=1.0):
    preds = (torch.sigmoid(logits) > thresh).float()
    inter = (preds * targets).sum((2, 3))
    den = preds.sum((2, 3)) + targets.sum((2, 3))
    return ((2 * inter + smooth) / (den + smooth)).mean().item()
...
def evaluate(model, loader, device, thresh=0.5):
    ...
    for x, y in loader:
        ...
        logits = model(x)
        dice += binary_dice_score(logits, y, thresh=thresh)
        iou += iou_score(logits, y, thresh=thresh)
    n = max(1, len(loader))
    return dice / n, iou / n
...
def _fill_holes(mask: np.ndarray) -> np.ndarray:
    """Fill holes via floodFill, no scipy required."""
    h, w = mask.shape
    border = np.zeros((h + 2, w + 2), dtype=np.uint8)
    inv = (mask == 0).astype(np.uint8) * 255  # background as 255
    cv2.floodFill(inv, border, seedPoint=(0, 0), newVal=0)
...
def _convex_hull(mask: np.ndarray) -> np.ndarray:
    ...
    for cnt in contours:
        ...
        cv2.drawContours(hull, [ch], -1, 1, thickness=-1)
    return hull
...
def postprocess_mask(mask: np.ndarray,
                     ...
                     use_hull: bool = False) -> np.ndarray:
    """
    mask: 0/1 uint8
    close_ks: odd int >=3 for closing. <=0 disables.
    """
    m = (mask > 0).astype(np.uint8)

    if isinstance(close_ks, int) and close_ks >= 3 and close_ks % 2 == 1:
        k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (close_ks, close_ks))
        m = cv2.morphologyEx(m, cv2.MORPH_CLOSE, k, iterations=max(1, int(close_iter)))
...
def train(args):
    ...
    for ep in range(1, args.epochs + 1):
        ...
        dice, iou = evaluate(model, val_loader, device, thresh=args.eval_thresh)
...
def _score_frames(probs: np.ndarray, mode: str, thresh: float, adaptive: bool, alpha: float, min_area: int):
    ...
    for i in range(M):
        ...
        if min_area > 0 and b.sum() < min_area:
            b[:] = 0

        p_masked_sum = float((p * (p >= t)).sum())  # 只累计高于阈值的概率
        if mode == "prob_sum":
            s = p_masked_sum
        elif mode == "bin_area":
            s = float(b.sum())
        elif mode == "hybrid":
            s = float(alpha * p_masked_sum + (1.0 - alpha) * b.sum())
        else:
    ...
    for img_path in sorted(input_dir.iterdir()):
        ...
        if suf in {".png", ".jpg", ".jpeg", ".tif", ".bmp"}:
            ...
            mask = postprocess_mask(
                mask,
                close_ks=int(getattr(args, "pp_close", 0)),
                close_iter=int(getattr(args, "pp_close_iter", 1)),
                fill_holes=bool(getattr(args, "pp_fill_holes", False)),
                use_hull=bool(getattr(args, "pp_hull", False)),
            )
        ...
        # ---------- 3D .mha volumes ----------
            ...
            print(f"[{img_path.name}] window=[{start_frame},{end_frame}) best={start_frame + int(np.argmax(scores))}")
            print("top frames:",
                  [(int(start_frame + i),
                    f"score={scores[i]:.0f}",
                    f"thr={used_thresh[i]:.2f}",
                    f"area={int(bin_masks[i].sum())}")
                   for i in top])
...
def get_args():
    ...
    t.add_argument("--eval_thresh", type=float, default=0.5, help="阈值化后计算验证指标的阈值")
    ...
    pr.add_argument("--pp_close", type=int, default=0,
                    help="闭运算核大小(奇数, 如 3/5/7)。<=0 关闭")
    pr.add_argument("--pp_close_iter", type=int, default=1,
                    help="闭运算迭代次数")
    pr.add_argument("--pp_fill_holes", action="store_true",
                    help="阈值后填洞")
    pr.add_argument("--pp_hull", action="store_true",
                    help="取凸包(让结果更圆/更规整)")
