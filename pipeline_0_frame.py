...
def predict_prob_tta(model, x, tta=False):
    """TTA：原图+水平翻转"""
    logits = model(x)
...
def predict(args):
    ...
    print(f"[i] spacing map loaded: {len(spacing_map)} cases")

    # ---------- global threshold ----------
    GLOBAL_THR = 0.48
    thr_cfg = Path("./checkpoints/thr.json")
    if thr_cfg.exists():
        GLOBAL_THR = float(json.load(open(thr_cfg))["best_thr"])
        print(f"[i] calibrated threshold = {GLOBAL_THR:.3f}")

    # ---------- model ----------
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model  = AttentionASPPUNet(base_c=args.base_c).to(device).eval()
    model.load_state_dict(torch.load(args.weights, map_location=device))
    ...
    rows = []                                          # AC 统计仅 2-D 帧
    empty_rows = []  # [NEW] GT全0帧的审计统计
    for img_path in tqdm(sorted(input_dir.iterdir()), desc="Cases", unit="case"):
        ext = img_path.suffix.lower()

        # ────── 单张 2-D 帧 ──────
        if ext in {".png", ".jpg", ".jpeg"}:
            sl   = cv2.imread(str(img_path), cv2.IMREAD_GRAYSCALE)
            sl_u = cv2.normalize(sl, None, 0, 255,
                                 cv2.NORM_MINMAX).astype(np.uint8)
            cla  = cv2.createCLAHE(1.0, (8, 8))
            enh  = cv2.medianBlur(cla.apply(sl_u), 3)
            x    = Compose([Resize(IMG_SIZE, IMG_SIZE), ToFloat(255),
                            ToTensorV2()])(image=enh)["image"].unsqueeze(0).to(device)

            prob = predict_prob_tta(model, x)          # 默认不开 TTA
            prob = cv2.resize(prob, (sl.shape[1], sl.shape[0]))
            ...
            frame_idx = int(img_path.stem.split("_s")[1])
        ...
        # ────── 3-D .mha ──────
            ...
            # --- 逐帧推理 ---
            for sl in tqdm(vol, desc=img_path.stem, unit="frame", leave=False):
                sl_u  = cv2.normalize(sl, None, 0, 255,
                                      cv2.NORM_MINMAX).astype(np.uint8)
                cla   = cv2.createCLAHE(1.0, (8, 8))
                enh   = cv2.medianBlur(cla.apply(sl_u), 3)
                ...
                prob  = cv2.resize(prob, (sl.shape[1], sl.shape[0]))
                prob  = cv2.GaussianBlur(prob, (5,5), 0)
                m_bin = refine_mask((prob > GLOBAL_THR).astype(np.uint8))
                pred_masks.append(m_bin)

            pred_stack = np.stack(pred_masks)                               # (N,H,W)

            # ---------- 用 GT 选帧 ----------
            if gt_dir is not None:
                gt_path = gt_dir / f"{img_path.stem}.mha"
                if not gt_path.exists():
                    raise FileNotFoundError(f"GT not found: {gt_path}")
                gt_vol  = sitk.GetArrayFromImage(sitk.ReadImage(str(gt_path)))
                gt_bin  = (gt_vol == 2).astype(np.uint8)
                ...
                if args.audit_empty:
                    ...
                    # 找出 GT 全0（无前景）的帧索引
                    idx0 = np.where(gt_bin.sum(axis=(1, 2)) == 0)[0]
                    if len(idx0) > 0:
                        vis_dir = out_dir / "empty_fp_vis"
                        vis_dir.mkdir(parents=True, exist_ok=True)

                        # 像素→面积换算（mm^2）
                        sx, sy = spacing_map[case_id]["spacing"]  # 注意 spacing_json 里是 [sx, sy]
                        px_area_mm2 = float(sx) * float(sy)

                        # 记录本例中“最大假阳性”的帧，方便可视化
                        worst_i, worst_area = -1, -1.0

                        pred_empty_cnt = 0
                        for i in idx0:
                            pred = (pred_stack[i] > 0).astype(np.uint8)  # (H,W) 0/1
                            pp = int(pred.sum())  # 预测为前景的像素数
                            area = pp * px_area_mm2  # mm^2
                            if pp == 0:
                                pred_empty_cnt += 1

                            # 连通域统计（可衡量“碎片噪声” vs “成块误检”）
                            num_labels, labels = cv2.connectedComponents(pred, connectivity=8)
                            comp_cnt = int(max(0, num_labels - 1))  # 去掉背景
                            # 最大连通域像素数
                            if comp_cnt > 0:
                                max_comp_pixels = int(np.max([(labels == k).sum() for k in range(1, num_labels)]))
                            else:
                                max_comp_pixels = 0
                            max_comp_area = max_comp_pixels * px_area_mm2

                            # 是否超过“较大假阳性面积阈值”
                            large_fp = int(area >= args.fp_area_thr_mm2)

                            empty_rows.append((
                                case_id, int(i), pp, round(area, 2),
                                comp_cnt, max_comp_pixels, round(max_comp_area, 2), large_fp
                            ))
                        ...
                        print(f"[empty] {case_id}: GT全0帧 {len(idx0)}，其中模型预测空帧 {pred_empty_cnt}，"
                              f"≥{args.fp_area_thr_mm2} mm² 的假阳性帧 {sum(r[-1] for r in empty_rows if r[0] == case_id)}")

                        # 保存“最糟假阳性帧”的可视化，便于人工排查
                        if worst_i >= 0 and worst_area > 0:
                            sl = vol[worst_i].astype(np.float32)
                            sl = cv2.normalize(sl, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
                            vis = cv2.cvtColor(sl, cv2.COLOR_GRAY2BGR)
                            cnts, _ = cv2.findContours((pred_stack[worst_i] > 0).astype(np.uint8),
                                                       cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                            cv2.drawContours(vis, cnts, -1, (0, 0, 255), 1)  # 红色勾边
                            cv2.imwrite(str(vis_dir / f"{case_id}_i{worst_i:03d}_FP_{worst_area:.1f}mm2.png"), vis)

                best_frame, best_dice = 0, -1.0
                for i in range(len(pred_stack)):
                    p, g = pred_stack[i], gt_bin[i]
                    inter = (p & g).sum()
                    dice  = 2 * inter / (p.sum() + g.sum() + 1e-7)
                    if dice > best_dice:
                        best_dice, best_frame = dice, i
                print(f"[i] {img_path.stem}: best by GT → frame {best_frame:03d}, Dice={best_dice:.3f}")
            else:
                best_frame = select_best_frame(pred_stack, topk=5)

            write_output_mha_and_json(pred_stack[best_frame],
                                      best_frame, img_path, out_dir)

        else:
            print(f"[warn] skip {img_path.name} (unsupported suffix)")

    # ---------- 汇总 CSV ----------
    if rows:
        csv_path = out_dir / "ac_results.csv"
        with open(csv_path, "w", newline="") as f:
            import csv; csv.writer(f).writerows([("case_id","frame_idx","ac_mm"), *rows])
        print(f"[✓] AC 结果已保存 → {csv_path}  (共 {len(rows)} 条)")
        # ---------- 汇总 CSV ----------
        # [NEW] 输出 GT 全0 审计结果
    if empty_rows:
        csv_path = out_dir / "empty_gt_audit.csv"
        with open(csv_path, "w", newline="") as f:
            import csv
            header = ("case_id", "frame_idx", "pred_pixels", "pred_area_mm2",
                      "components", "largest_comp_pixels", "largest_comp_area_mm2", "large_fp_flag")
            csv.writer(f).writerows([header, *empty_rows])
        print(f"[✓] 空帧审计已保存 → {csv_path}  (共 {len(empty_rows)} 条)")
...
def get_args():
    ...
    pr.add_argument("--gt_dir", default=None,
                    help="若指定，用 GT mask 的 Dice 选择最佳帧（路径指向 *.mha GT 标签文件夹）")
    pr.add_argument("--audit_empty", action="store_true",
                        help="当提供 --gt_dir 时，统计 GT 全0帧上的新模型假阳性情况，并输出 CSV/可视化")
    pr.add_argument("--fp_area_thr_mm2", type=float, default=50.0,
                        help="把预测面积超过该阈值(mm^2)的帧标记为大假阳性")
