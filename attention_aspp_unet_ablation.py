...
class AttentionASPPUNet(nn.Module):
    def __init__(self, in_channels=1, num_classes=1,
                 base_c=32, use_att=True, use_aspp=True):
        super().__init__()
        # 桥接层
        if use_aspp:
            ...
            self.bridge = nn.Sequential(
                ConvBNReLU(base_c * 8, base_c * 16),
                ConvBNReLU(base_c * 16, base_c * 16)
            )
        ...
        self.bridge = ASPP(base_c*8, base_c*16)
        # UpBlock 时只在前三层用注意力
        self.u4 = UpBlock(base_c * 16, base_c * 8, use_att=use_att)
        self.u3 = UpBlock(base_c * 8, base_c * 4, use_att=use_att)
        self.u2 = UpBlock(base_c * 4, base_c * 2, use_att=use_att)
        self.u1 = UpBlock(base_c * 2, base_c, use_att=False)  # 仍保持关闭
...
class FetalACDataset(Dataset):
    def __init__(self,
                 img_paths: List[Path],
                 msk_paths: List[Path] | None,
                 train: bool = True,
                 cfg=None):  # ← 新增 cfg，用来拿 args
        self.img_paths = img_paths
        self.msk_paths = msk_paths
        self.train = train
        self.cfg = cfg  # 保存 CLI 配置
        self.transform = self._build_transform()

    def _build_transform(self):
        """
        根据 CLI 开关动态构造 Albumentations pipeline

        - cfg.aug = "full"  : 保持原有“重”增强
        - cfg.aug = "light" : 只做 Resize + Flip
        - cfg.no_clahe=True : 去掉 CLAHE + MedianBlur（无论 full / light）
        """
        ...
        if self.train:
            # 决定增强强度
            aug_mode = getattr(self.cfg, "aug", "full") if self.cfg else "full"
            t_train: list = []
            ...
            # 可选去掉 CLAHE + MedianBlur
            if getattr(self.cfg, "no_clahe", False):
                t_train = [tr for tr in t_train if not isinstance(tr, SafeCLAHE)]
                t_train = [tr for tr in t_train if not isinstance(tr, SafeMedianBlur)]
            ...
            return Compose(t_train)

        # ------------------- 验证 / 推理增强 -------------------
        t_val = [Resize(IMG_SIZE, IMG_SIZE)]

        if not getattr(self.cfg, "no_clahe", False):
            # 只有在没显式 --no_clahe 时才加
            t_val += [
                SafeCLAHE(1.0, (8, 8)),
                SafeMedianBlur(3)
...
def train(args):
    ...
    root_train = Path(args.data_dir)  # 训练集
    root_val = Path(args.val_dir)  # 验证集
    ...
    if (root_val/"images").exists():
        ...
        train_ds = FetalACDataset(train_imgs, train_msks, train=True,  cfg=args)
        val_ds   = FetalACDataset(val_imgs, val_msks, train=False,  cfg=args)
    else:
...
def predict_prob_tta(model, x, use_tta=True):
    logits = model(x)
    if use_tta:
        x_flip = torch.flip(x, dims=[-1])
        logits_flip = torch.flip(model(x_flip), dims=[-1])
...
def calibrate(args):
    ...
    for thr in thrs:
        ...
        for p in img_paths:
            ...
            prob = predict_prob_tta(model, x,use_tta = (not args.no_tta))
...
def predict(args):
    ...
    for img_path in sorted(input_dir.iterdir()):
        ...
        if ext in {'.png', '.jpg', '.jpeg'}:
            ...
            prob = cv2.GaussianBlur(prob, (5, 5), 0)
            if args.no_post:
                mask = (prob > GLOBAL_THR).astype(np.uint8)
            else:
            ...
            for sl in vol:
                ...
                prob = predict_prob_tta(model, x,use_tta = (not args.no_tta))                        # (H',W')
...
# 消融实验参数
    t.add_argument("--no_attn", action="store_true",
                   help="关闭跳接 AttentionGate")
    t.add_argument("--no_aspp", action="store_true",
                   help="桥接层不用 ASPP 而用两层 3×3 Conv")
    t.add_argument("--no_tta", action="store_true",
                   help="推理时不做 TTA（只用原图）")
    t.add_argument("--no_post", action="store_true",
                   help="禁用 refine_mask 后处理")
    t.add_argument("--aug", choices=["full", "light"], default="full",
                   help="数据增强方案：full=默认；light=弱增强")
    t.add_argument("--no_clahe", action="store_true",
                   help="增强里去掉 CLAHE + MedianBlur")
    ...
    # 消融实验参数
    pr.add_argument("--no_tta", action="store_true")
    pr.add_argument("--no_post", action="store_true")
    pr.add_argument("--fix_thr", type=float,
                    help="若指定则直接用该阈值，不加载 thr.json")
