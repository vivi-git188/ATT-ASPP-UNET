# -*- coding: utf-8 -*-
"""
Attention-ASPP-UNet – unified train / predict / calibrate script (Ablation-ready)
消融开关：
  --no_att        关闭所有 Attention Gate
  --no_aspp       关闭 ASPP（用简单 3×3 conv 替代）
  --no_edge_loss  关闭边界损失（训练期）
...
class UpBlock(nn.Module):
    def __init__(self,in_c,out_c,use_att=True):
        ...
        if isinstance(self.att,type):
            self.att = self.att(out_c,out_c,out_c//2)
...
class AttentionASPPUNet(nn.Module):
    def __init__(self,in_channels=1,num_classes=1,base_c=32,
        ...
        if use_aspp:
            ...
            self.bridge=nn.Sequential(ConvBNReLU(base_c*8,base_c*16,3),nn.Dropout(0.1))
...
def load_state_dict_compat(model: nn.Module, ckpt_path: Union[str, Path]):
    ...
    new_sd = {}
    for k, v in sd.items():
        nk = str(k).replace(".W_g.", ".Wg.").replace(".W_x.", ".Wx.")
        new_sd[nk] = v
...
class FetalACDataset(Dataset):
    ...
    def _tfm(self):
        ...
        if self.train:
            t=[ Resize(IMG_SIZE,IMG_SIZE),
                ...
                ElasticTransform(alpha=8, sigma=3, alpha_affine=0, p=0.25),
                SafeCLAHE(1.0,(8,8)),
                SafeMedianBlur(3),
    ...
    def _read(p:Path):
        if p.suffix.lower()=='.mha':
            assert sitk is not None, "Need SimpleITK to read .mha"
    ...
    def __getitem__(self,i):
        ...
        x = s["image"].float()
        y = (s["mask"].unsqueeze(0).float()/255.0)
        return x, y
...
def build_criterion(base, edge, edge_w, args):
    def crit(l,t):
        ...
        if len(pos_idx)>0:
            ...
            if edge_w>0: edge_loss=edge(l[pos_idx],t[pos_idx])*edge_w
...
def refine_mask(m):
    ...
    cmax=np.argmax(bc); m=(lab2==cmax).astype(np.uint8)
...
def _circularity_score(mask01: np.ndarray)->float:
...
def select_best(pred_stack,topk=5):
    if len(pred_stack)==0: return 0
    areas=np.array([(p>0).sum() for p in pred_stack])
    idx=areas.argsort()[::-1][:max(1,min(topk,len(areas)))]
    return int(max(idx, key=lambda i:_circularity_score(pred_stack[i])))
...
def train(args):
    ...
    if args.val_dir:
        ...
        rng = np.random.default_rng(args.seed); rng.shuffle(candidate_idx)
    ...
    if args.stage=="finetune":
        if not args.pretrained:
            raise ValueError("finetune 阶段需要 --pretrained")
    ...
    edge_loss=EdgeLoss()
    edge_w = 0.0 if args.no_edge_loss else args.edge_w
    crit=build_criterion(base_loss, edge_loss, edge_w, args)
...
def predict(args):
    ...
    def _spacing_from_map(case_id: str) -> Optional[Tuple[float, float]]:
        if case_id not in spacing_map: return None
    ...
    for p in tqdm(sorted(inp.iterdir()), desc="Processing files", unit="file"):
        ...
        # ---------------- MHA ----------------
            ...
            preds = []
            for sl in tqdm(vol, desc=f"{p.name} slices", unit="slice", total=vol.shape[0], leave=False):
                sl_u8 = cv2.normalize(sl, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
                e = cv2.medianBlur(cv2.createCLAHE(1.0, (8, 8)).apply(sl_u8), 3)
                x = Compose([Resize(IMG_SIZE, IMG_SIZE), ToFloat(max_value=255), ToTensorV2()])(image=e)['image'].unsqueeze(0).to(device)
                prob = predict_prob_tta(model, x)
...
# ---------- CLI ----------
def get_args():
    p=argparse.ArgumentParser("A-ASPP-UNet unified (ablation)"); sp=p.add_subparsers(dest="cmd",required=True)
    ...
    t.add_argument("--neg_dir")
    t.add_argument("--val_dir")
    t.add_argument("--output_dir",default="./checkpoints")
    t.add_argument("--pretrained")
    ...
    t.add_argument("--edge_w",type=float,default=0.05)
    t.add_argument("--neg_bce_w",type=float,default=0.05)
    # ablation
    t.add_argument("--no_att", action="store_true", help="关闭 Attention Gate")
    t.add_argument("--no_aspp", action="store_true", help="关闭 ASPP")
    t.add_argument("--no_edge_loss", action="store_true", help="关闭 EdgeLoss")
    ...
    pr.add_argument("--spacing_json",required=True, help="PNG 的 spacing 映射 JSON")
    pr.add_argument("--base_c",type=int,default=48)
    pr.add_argument("--no_att", action="store_true")
    pr.add_argument("--no_aspp", action="store_true")
    ...
    ca.add_argument("--no_att", action="store_true")
    ca.add_argument("--no_aspp", action="store_true")
