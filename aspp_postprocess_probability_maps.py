...
def postprocess_single_probability_map(softmax_prob_map, configs):
    th = configs.get("soft_threshold", 0.5)

    # ---- 情况 A：单通道二分类 ----
    if softmax_prob_map.ndim == 3:
        # (N,H,W) → (N,H,W) binary mask
        masks = (softmax_prob_map >= th).astype(np.uint8)
        return masks

    # ---- 情况 B：多通道（nnUNet 原逻辑）----
    labels_dict = dict(optimal=1, suboptimal=2)
    softmax_maps = softmax_prob_map.copy()
    softmax_maps[softmax_maps < th] = 0
    masks = np.argmax(softmax_maps, axis=0)         # (N,H,W)
